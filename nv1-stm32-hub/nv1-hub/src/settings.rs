...
pub fn flash_write(
    ...
    f.blocking_erase(FLASH_SETTINGS_ADDRESS, FLASH_SETTINGS_ADDRESS + FLASH_SETTINGS_SIZE)?;
    f.blocking_write(FLASH_SETTINGS_ADDRESS, &buf)?;

    Ok(())
}

pub fn validate_and_fix_settings(settings: &mut Settings, flash: &mut Flash<'_, Blocking>) -> Result<(), embassy_stm32::flash::Error> {
